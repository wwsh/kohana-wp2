...
class Kohana_Cache_File extends Cache implements Cache_GarbageCollect
{
    ...
    protected function __construct(array $config)
    {
        ...
        if ($this->_cache_dir->isFile()) {
            throw new Cache_Exception('Unable to create cache directory as a file already exists : :resource',
                                      array(':resource' => $this->_cache_dir->getRealPath()));
        }

        // Check the read status of the directory
        if (!$this->_cache_dir->isReadable()) {
            throw new Cache_Exception('Unable to read from the cache directory :resource',
                                      array(':resource' => $this->_cache_dir->getRealPath()));
        }

        // Check the write status of the directory
        if (!$this->_cache_dir->isWritable()) {
            throw new Cache_Exception('Unable to write to the cache directory :resource',
                                      array(':resource' => $this->_cache_dir->getRealPath()));
        }
    }
    ...
    public function set($id, $data, $lifetime = null)
    {
        ...
        if (!$dir->isDir()) {
            ...
            if (!mkdir($directory, 0777, true)) {
                throw new Cache_Exception(__METHOD__ . ' unable to create directory : :directory',
        ...
        try {
            ...
            if ($file->isFile()) {
                try {
                    ...
                    if ($e->getCode() === E_WARNING) {
                        throw new Cache_Exception(__METHOD__ . ' failed to delete file : :file',
            ...
            elseif ($file->isDir()) {
                ...
                try {
                    ...
                    if ($e->getCode() === E_WARNING) {
                        throw new Cache_Exception(__METHOD__ . ' failed to delete directory : :directory',
                                                  array(':directory' => $file->getRealPath()));
                    }
                    throw $e;
                }
            } else {
    ...
    protected function _make_directory($directory, $mode = 0777, $recursive = false, $context = null)
    {
        if (!mkdir($directory, $mode, $recursive, $context)) {
            throw new Cache_Exception('Failed to create the defined cache directory : :directory',
                                      array(':directory' => $directory));
        }
