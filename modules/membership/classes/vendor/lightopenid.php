...
class Vendor_LightOpenID
{
    ...
    {
        preg_match_all("#<{$tag}[^>]*$attrName=['\"].*?$attrValue.*?['\"][^>]*$valueName=['\"](.+?)['\"][^>]*/?>#i",
                       $content, $matches1);
        preg_match_all("#<{$tag}[^>]*$valueName=['\"](.+?)['\"][^>]*$attrName=['\"].*?$attrValue.*?['\"][^>]*/?>#i",
                       $content, $matches2);
            ...
            if ($yadis) {
                ...
                foreach ($headers as $header) {
                    ...
                    if (preg_match('#Content-Type\s*:\s*application/xrds\+xml#i', $header)) {
                        ...
                        if (preg_match('#<Service.*?>(.*)<Type>\s*' . $ns . '(.*?)\s*</Type>(.*)</Service>#s', $content,
                                       $m)) {
