...
class Kotal_View extends Kohana_View
{
     ...
     */
    protected static $_tal_exclude = null; // bugfix
...
//		}
    ...
    public function set_filename($file)
    {
        ...
        if (($path = Kohana::find_file('views', $file, Kohana::$config->load('kotal.ext'))) === false) {
    ...
    protected function _check_tal_exclusions($clear = false)
    {
        ...
        $controller = Request::current() !== null ? UTF8::strtolower(Request::current()->controller()) : '';
        ...
        if (in_array($controller, self::$_tal_exclude) OR strpos($this->_file, MODPATH) === 0 OR strpos($this->_file,
                                                                                                        SYSPATH) === 0
        ) {
            $this->use_tal(false);
        }
