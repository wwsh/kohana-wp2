    ...
    final public function _executeMacroOfTemplate($path, PHPTAL $local_tpl)
    {
        ...
        if (preg_match('/^(.*?)\/([a-z0-9_-]*)$/i', $path, $m)) {
            ...
            if (!function_exists($fun)) {
                throw new PHPTAL_MacroMissingException("Macro '$macroName' is not defined in $file",
            ...
            if (!function_exists($fun)) {
                throw new PHPTAL_MacroMissingException("Macro '$path' is not defined",
                                                       $local_tpl->_source->getRealPath());
            }
            $fun($local_tpl, $this);
        }
    }
...
/**
 * Custom date modifier for TAL
 * @param  $src
 * @param  $nothrow
 * @return string
 */

function phptal_tales_date($src, $nothrow)
{
    if (!$src) {
        return $src;
    }
    ...
    if (strpos($src, '#') === false) {
        return "Helper_Format::date(" . PHPTAL_Php_TalesInternal::path($src) . ")";
    }

    $a = explode('#', $src);
    return "date('${a[0]}',strtotime(" . PHPTAL_Php_TalesInternal::path($a[1]) . "))";

}

function phptal_tales_substrws($src, $nothrow)
{
    if (!$src) {
        return $src;
    }
    $a = explode('#', $src);
    return "mb_substrws(" . PHPTAL_Php_TalesInternal::path($a[0]) . "," . $a[1] . ")";
}
...
/**
 * Displaying ip from binary format.
 *
 * @param  $src
 * @param  $nothrow
 * @return string
 */
function phptal_tales_ip($src, $nothrow)
{
    if (!$src) {
        return $src;
    }

    return "Helper_Format::bin2numIp(" . PHPTAL_Php_TalesInternal::path(trim($src)) . ")";
 ...
 * @return string
 */
function phptal_tales_duration($src, $nothrow)
{
    if (!$src) {
        return $src;
    }

    return "Helper_Format::duration(" . PHPTAL_Php_TalesInternal::path(trim($src)) . ")";

}

/**
 * Displaying the past time.
 * @param $src
 * @param $nothrow
 * @return string
 */
function phptal_tales_pasttime($src, $nothrow)
{
    if (!$src) {
        return $src;
    }

    return "Helper_Format::relative_time(" . PHPTAL_Php_TalesInternal::path(trim($src)) . ")";
 ...
 * @return string
 */
function phptal_tales_songduration($src, $nothrow)
{
    if (!$src) {
        return $src;
    }

    return "Helper_Format::songduration(" . PHPTAL_Php_TalesInternal::path(trim($src)) . ")";
