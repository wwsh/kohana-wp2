...
class PHPTAL_Dom_Element extends PHPTAL_Dom_Node implements PHPTAL_Php_Tree
{
    ...
    public function __construct($qname, $namespace_uri, array $attribute_nodes, PHPTAL_Dom_XmlnsState $xmlns)
    {
        ...
        foreach ($attribute_nodes as $index => $attr) {
            ...
            if ($this->xmlns->isValidAttributeNS($namespace_uri, $attr->getQualifiedName())) {
                $this->attribute_nodes[$index] = new PHPTAL_Dom_Attr($attr->getQualifiedName(), $namespace_uri,
                                                                     $attr->getValueEscaped(), $attr->getEncoding());
            }
        }
    ...
    {
        $isCDATAelement = PHPTAL_Dom_Defs::getInstance()->isCDATAElementInHTML($this->getNamespaceURI(),
                                                                               $this->getLocalName());
    ...
    public function getOrCreateAttributeNode($qname)
    {
        ...
        $attr                    = new PHPTAL_Dom_Attr($qname, "", null,
                                                       'UTF-8'); // FIXME: should find namespace and encoding
    ...
    public function setAttributeNS($namespace_uri, $qname, $value)
    {
        ...
        if (!($n = $this->getAttributeNodeNS($namespace_uri, $localname))) {
            $this->attribute_nodes[] = $n = new PHPTAL_Dom_Attr($qname, $namespace_uri, null,
                                                                'UTF-8'); // FIXME: find encoding
        }
        $n->setValue($value);
    }
    ...
    public function generateContent(PHPTAL_Php_CodeWriter $codewriter = null, $realContent = false)
    {
        // For backwards compatibility only!
        if ($codewriter === null) {
            $codewriter = self::$_codewriter_bc_hack_;
        } // FIXME!
    ...
    {
        return (($mode === PHPTAL::XHTML || $mode === PHPTAL::HTML5) && PHPTAL_Dom_Defs::getInstance()->isEmptyTagNS($this->getNamespaceURI(),
                                                                                                                     $this->getLocalName())) ||
    ...
    private function separateAttributes()
    {
        ...
        foreach ($this->attribute_nodes as $index => $attr) {
            ...
            if (PHPTAL_Dom_Defs::getInstance()->isHandledXmlNs($attr->getQualifiedName(), $attr->getValueEscaped())) {
                ...
                if ($this->xmlns->isHandledNamespace($attr->getNamespaceURI())) {
                    $talAttributes[$attr->getQualifiedName()] = $attr;
                    $attr->hide();
                } else {
                    if (PHPTAL_Dom_Defs::getInstance()->isBooleanAttribute($attr->getQualifiedName())) {
    ...
    private function orderTalAttributes(array $talAttributes)
    {
        ...
        foreach ($talAttributes as $key => $domattr) {
            $nsattr = PHPTAL_Dom_Defs::getInstance()->getNamespaceAttribute($domattr->getNamespaceURI(),
                                                                            $domattr->getLocalName());
        ...
        foreach ($temp as $prio => $dat) {
            ...
            if ($nsattr instanceof PHPTAL_NamespaceAttributeSurround) {
                ...
                if ($nsattr instanceof PHPTAL_NamespaceAttributeReplace) {
                    $this->replaceAttributes[] = $handler;
                } else {
                    if ($nsattr instanceof PHPTAL_NamespaceAttributeContent) {
