...
class PHPTAL_Dom_SaxXmlParser
{
            ...
            for (; $i < $len; $i++) {
                ...
                switch ($state) {
                    ...
                    case self::ST_TEXT:
                        if ($c === '<') {
                            if ($mark != $i) {
                                $builder->onElementData($this->sanitizeEscapedText($this->checkEncoding(substr($src,
                                                                                                               $mark,
                                                                                                               $i - $mark))));
                            }
                            $mark  = $i;
                    ...
                    case self::ST_COMMENT:
                        if ($c === '>' && $i > $mark + 4 && substr($src, $i - 2, 2) === '--') {
                            ...
                            if (preg_match('/^-|--|-$/', substr($src, $mark + 4, $i - $mark + 1 - 7))) {
                                $this->raiseError("Ill-formed comment. XML comments are not allowed to contain '--' or start/end with '-': " . substr($src,
                                                                                                                                                      $mark + 4,
                                                                                                                                                      $i - $mark + 1 - 7));
                            }
                    ...
                    case self::ST_PREPROC:
                        if ($c === '>' and $src[$i - 1] === '?') {
                            $builder->onProcessingInstruction($this->checkEncoding(substr($src, $mark,
                                                                                          $i - $mark + 1)));
                            $mark  = $i + 1; // mark text start
                            $state = self::ST_TEXT;
                        }
                    ...
                    case self::ST_ATTR_QUOTE:
                        if ($c === $quoteStyle) {
                            $attributes[$attribute] = $this->sanitizeEscapedText($this->checkEncoding(substr($src,
                                                                                                             $mark,
                                                                                                             $i - $mark)));
    ...
    private function sanitizeEscapedText($str)
    {
        ...
        */
        ...
        $str = strtr(preg_replace('/&(?!(?:#x?[a-f0-9]+|[a-z][a-z0-9]*);)/i', '&amp;', $str),
                     array('<' => '&lt;', ']]>' => ']]&gt;'));
